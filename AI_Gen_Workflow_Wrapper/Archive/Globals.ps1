...
function Load-MigrationConfig {
    if (Test-Path -Path $script:config.ConfigPath) {
        try {
            ...
            if ($configData.migrationScripts) {
                $script:migrationScripts = $configData.migrationScripts
            }
            
            Write-Log "Configuration loaded successfully from $($script:config.ConfigPath)"
            return $true
        }
        catch {
...
function Save-MigrationConfig {
    try {
        ...
        $configData = @{
            ...
            migrationScripts = $script:migrationScripts
        }
        ...
        Write-Log "Configuration saved successfully to $($script:config.ConfigPath)"
        return $true
    }
    catch {
