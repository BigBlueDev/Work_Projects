    ...
    foreach ($folderName in $Path.Split('\')) {
        ...
        if (-not $nextFolder) {
            ...
            try {
                ...
                Write-Log "ERROR: Failed to create folder '$folderName' under '$($currentFolder.Name)'. Error: $($_.Exception.Message)" "ERROR"
                throw
            }
        }
...
try {
    ...
    foreach ($VMToMigrateName in $VMs) {
        ...
        try {
            ...
            if ($Clone) {
    # Clone Task Progress Tracking (Adapt as needed - New-VM might not have the same progress info)
    Write-Log "Cloning VM '$($VMToMigrateName)' to destination vCenter..."
    try {
        $cloneTask = New-VM -VM $vmToMigrate `
            ...
            -RunAsync `
            -ErrorAction Stop
        Write-Log "Clone Task Started, waiting for completion"

        while ($cloneTask.ExtensionData.Info.State -eq "running") {
            $progress = $cloneTask.ExtensionData.Info.Progress
            Write-Log "Cloning Progress: $($progress)" -Level "DEBUG"
            Write-Progress -Activity "Cloning VM '$($VMToMigrateName)'" -Status "Progress: $($progress)%" -PercentComplete $progress
            Start-Sleep -Seconds 5 # Adjust sleep time as needed
            $cloneTask.ExtensionData.UpdateViewData("Info.State", "Info.Progress")
        }
        Write-Log "VM cloned successfully."
        $cloneTask | Wait-Task # Ensure task is fully completed
    }
    catch {
    ...
    try {
        $moveTask = Move-VM -VM $vmToMigrate `
            -Destination $destHost `
            -Datastore $destDatastore `
            -NetworkAdapter $networkMapping.Adapter `
            -PortGroup $networkMapping.PortGroup `
            ...
            -RunAsync `
            -ErrorAction Stop
        Write-Log "Cross-vCenter vMotion Task Started, waiting for completion..."

        while ($moveTask.ExtensionData.Info.State -eq "running") {
            $progress = $moveTask.ExtensionData.Info.Progress
            Write-Log "Migration Progress: $($progress)" -Level "DEBUG"
            Write-Progress -Activity "Migrating VM '$($VMToMigrateName)'" -Status "Progress: $($progress)%" -PercentComplete $progress
            Start-Sleep -Seconds 5 # Adjust sleep time as needed
            $moveTask.ExtensionData.UpdateViewData("Info.State", "Info.Progress")
        }
        Write-Log "Cross-vCenter vMotion completed successfully for VM '$($VMToMigrateName)'."
        $moveTask | Wait-Task # Ensure task is fully completed
